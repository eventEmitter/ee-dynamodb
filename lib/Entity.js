	...
	var Entity = new Class( {
			...
			this.request( "CreateTable", table, function( err, status, data ){
				if ( !err ){
					...
					if ( err.name === "LimitExceeded" ){
						setTimeout( function(){
							this.$create( null, callback, table );
						}.bind( this ), 5000 );
					}
					callback( err );
					resumeCreate();
		...
		, $delete: function( callback ){
				...
				setTimeout( function(){								
					this.$describe( function( err, tableInfo ){
						if ( err ) {
							...
							if ( tableInfo.status === "active" ) this.$delete( callback );
							else wait();
						}
					}.bind( this ) );
				}.bind( this ), 5000 );
			...
			this.request( "DeleteTable", { TableName: this.tableName }, function( err, status, data ){
				if ( err ){
					if ( err.name === "ResourceNotFound" ) callback();
		...
		, $update: function( read, write, callback ){
			callback = callback || function(){};
					...
					this.$describe( function( err, tableInfo ){
						if ( err ) callback( err );
							...
							if ( waitForFinished ) {
								...
								if ( tableInfo.status === "active" ) this.$update( read, write, callback );
								else wait();
							}
						}
					}.bind( this ) );
