	...
	module.exports = new Class( {
		...
		, init: function( options ){
			...
			this.isNewRecord 	= !options.attribute;
			this.db 			= options.db;
		}
		...
		, typeEncoder: function( val ){
			if ( typeof val === "string" && val.length > 0 ) return { "S": val };
				...
				else if ( typeof val[ 0 ] === "number" ) return { NN: val };
				if ( Buffer.isBuffer( val[ 0 ] ) ) return { BB: val.map( function( v ){ return v.toString( "base64" ); } ) };
			}
			return null;	
		}
